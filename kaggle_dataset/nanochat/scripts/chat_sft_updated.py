...
parser.add_argument("--warmup-datasets", type=int, default=1, help="warm up/cache remote SFT datasets on rank 0 before distributed loading (1=yes, 0=no)")
args = parser.parse_args()
user_config = vars(args).copy()
# -----------------------------------------------------------------------------

def warmup_sft_datasets(base_dir, ddp, master_process):
    """Warm up Hugging Face-backed datasets once on rank 0.

    This keeps the training mixture identical to scripts.chat_sft, but avoids
    first-touch remote downloads/cache creation happening concurrently across
    multiple distributed workers, which can be fragile in notebook runtimes.
    """
    lock_path = os.path.join(base_dir, "hf_sft_dataset_warmup.lock")
    warmup_specs = [
        ...
        ("MMLU test", lambda: MMLU(subset="all", split="test", stop=5200)),
        ("GSM8K train", lambda: GSM8K(subset="main", split="train")),
        ("GSM8K test", lambda: GSM8K(subset="main", split="test", stop=420)),
    ]
    if master_process:
        print0("Starting single-rank SFT dataset warmup...")
        with FileLock(lock_path):
            for label, builder in warmup_specs:
                t0 = time.time()
                dataset = builder()
                print0(f"Warmup complete: {label} ({len(dataset):,} rows) in {time.time() - t0:.2f}s")
                del dataset
        print0("SFT dataset warmup finished on rank 0.")
    if ddp:
        print0("Waiting at post-warmup barrier...")
        dist.barrier()
...
# Warm up/cache remote datasets once before all ranks instantiate them.
# This keeps the exact same SFT task mixture as scripts.chat_sft while making
# startup more robust in environments like Kaggle notebooks.
if args.warmup_datasets:
    warmup_sft_datasets(base_dir, ddp, master_process)
